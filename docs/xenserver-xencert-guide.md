...
The purpose of this document is to familiarize the reader with shared storage certification kit, the XenServer Storage Certification Kit. The certification kit is designed to certify the interoperability of various types of storage hardware with XenServer.  
...
For the installation the supplemental pack shared-storage-certification-kit.iso needs to be transferred to the control domain, Dom0, of the host under test using either wget or scp.  Copy the ISO onto the /root directory.

The supplemental pack subsequently needs to be installed using the following command:  

    xe update-upload file-name=”/root/shared-storage-certification-kit.iso”  
